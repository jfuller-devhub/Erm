...
import {
  ...
  COMMENT_TYPE_LABELS,
  COMMENT_TYPE_STYLES,
} from '../../data/billCommentData';
import { BillCommentFormModal } from './BillCommentFormModal';

interface BillActivityTabProps {
  bill: Bill;
}

export function BillActivityTab({ bill }: BillActivityTabProps) {
  const [comments, setComments] = useState<BillComment[]>([]);
  const [topLevelComments, setTopLevelComments] = useState<BillComment[]>([]);
  const [addModalOpen, setAddModalOpen] = useState(false);
  const [replyTo, setReplyTo] = useState<BillComment | null>(null);
  const [filterType, setFilterType] = useState<CommentType | 'all'>('all');

  useEffect(() => {
    const allComments = loadBillComments();
    setComments(allComments);
    const topLevel = getTopLevelComments(allComments, bill.id);
    setTopLevelComments(topLevel);
  }, [bill.id]);

  function handleAddComment(data: Omit<BillComment, 'id' | 'createdAt' | 'createdBy' | 'updatedAt' | 'updatedBy'>) {
    const allComments = loadBillComments();
    const newComment = createComment(allComments, data);
    const updated = [...allComments, newComment];
    saveBillComments(updated);
    setComments(updated);
    setTopLevelComments(getTopLevelComments(updated, bill.id));
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          >
            Legislative tracking, analysis, position decisions, and internal discussions
          </p>
        </div>
        <button
